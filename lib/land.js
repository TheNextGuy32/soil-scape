...
exports.createBlobbyLandmass = (ctx) => {
  for (let z = 0; z < ctx.area; z++) {
    ctx.datasets.continent.value[z] = false;
  }
  ...
  for (let b = 0; b < 3; b++) {
    ...
    const newContinent = createBlob(ctx, startingCoord, ctx.columns / 10, 6);
    for (let p = 0; p < newContinent.length; p++) {
      if (!ctx.datasets.continent.value[newContinent[p]]) {
        ctx.datasets.height.value[newContinent[p]] += utility.randomNumberBetween(2, 5);
        ctx.datasets.continent.value[newContinent[p]] = true;
      }
    }
  }
};
exports.createLandmass = (ctx) => {
  for (let z = 0; z < ctx.area; z++) {
    ctx.datasets.continent.value[z] = 0;
  }
  ...
  for (let c = 0; c < 3; c++) {
    ...
    while (centerContinentX === -1 || centerContinentY === -1) {
      ...
      if (ctx.datasets.continent.value[ctx.convertToZ({ f: attemptedX, s: attemptedY })] === 0) {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      for (let p = 0; p < blobResults.length; p++) {
        if (!ctx.datasets.continent.value[blobResults[p]]) {
          ctx.datasets.height.value[blobResults[p]] += utility.randomNumberBetween(2, 5);
          ctx.datasets.continent.value[blobResults[p]] = true;
        }
      }
    }
  }
